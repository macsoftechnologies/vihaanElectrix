...
export class VEnergyService {
  constructor(
    @InjectModel(vEnergy.name) private vEnergyModel: Model<vEnergy>,
    @InjectModel(vEnergySpecs.name)
    private vEnergySpecsModel: Model<vEnergySpecs>,
  ) {}
  ...
  async updateCharger(req: vEnergySpecsDto, image) {
    try {
      ...
      console.log(req);
      // return false;
      //const createVehicleResp = await this.vEnergySpecsModel.updateOne({ chargerId: req.chargerId},{$set:{chargerImage: req.chargerImage, powerSpecifications: req.powerSpecifications, latitude: req.location.latitude, longitude: req.location.longitude}})

      const createVehicleResp = await this.vEnergySpecsModel.updateOne(
        { chargerId: req.chargerId },
        {
          $set: {
            chargerImage: req.chargerImage,
            powerSpecifications: req.powerSpecifications,
  ...
  async getChargerDetails(req: vEnergySpecsDto) {
    try {
      const vehicleResponse = await this.vEnergySpecsModel.find({
        chargerId: req.chargerId,
      });
      if (vehicleResponse) {
        return {
  ...
  async findVehicle(chargerId: string) {
    try {
      const vehicleResponse = await this.vEnergySpecsModel.findOne({
        chargerId: chargerId,
      });
      if (vehicleResponse) {
        return {
  ...
  async delete(body: vEnergySpecsDto) {
    try {
      console.log(body);
      const deleteRes = await this.vEnergySpecsModel.deleteOne({
